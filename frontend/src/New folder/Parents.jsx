                ...
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <Select value={filterParentType} onValueChange={setFilterParentType}>
                        ...
                        <SelectContent className="z-50 rounded-xl border border-stone-200 bg-white shadow-lg text-sm text-stone-900 p-1">
                            <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="all">All Parents</SelectItem>
                            <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="Father">Fathers</SelectItem>
                            <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="Mother">Mothers</SelectItem>
                            <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="Guardian">Guardians</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            </div>
                        ...
                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <Select value={newParent.parent} onValueChange={v => handleSelectChange('parent', v)}>
                                    <SelectTrigger className="h-10 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-[#B8975A] focus:border-[#B8975A] hover:border-[#B8975A] transition-colors"><SelectValue placeholder="Select parent type" /></SelectTrigger>
                                    <SelectContent className="z-50 rounded-xl border border-stone-200 bg-white shadow-lg text-sm text-stone-900 p-1">
                                        <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="Father">Father</SelectItem>
                                        <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="Mother">Mother</SelectItem>
                                        <SelectItem className="rounded-lg px-3 py-2 cursor-pointer hover:bg-amber-50 focus:bg-amber-50 text-stone-800 font-medium" value="Guardian">Guardian</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            {[
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="rounded-xl border border-stone-200 max-h-40 overflow-y-auto divide-y divide-stone-50">
                                    {mentees.length === 0 ? (
                                        <p className="text-xs text-stone-400 p-3">No mentees available</p>
                                    ) : mentees.map(m => {
                                        const checked = newParent.mentee.includes(m._id);
                                        return (
                                            <label key={m._id} className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors hover:bg-stone-50 ${checked ? 'bg-amber-50/60' : ''}`}>
                                                <input type="checkbox" checked={checked}
                                                    onChange={() => {
                                                        const current = newParent.mentee;
                                                        const updated = checked ? current.filter(id => id !== m._id) : [...current, m._id];
                                                        handleSelectChange('mentee', updated);
                                                    }}
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    {m.admissionNumber && <p className="text-xs font-mono text-stone-400">{m.admissionNumber}</p>}
                                                </div>
                                            </label>
                                        );
                                    })}
                                </div>
                                {newParent.mentee.length > 0 && (
                                    <p className="text-xs font-semibold mt-1" style={{ color: GOLD }}>
                                        {newParent.mentee.length} mentee{newParent.mentee.length > 1 ? 's' : ''} selected
                                    </p>
                                )}
                            </div>
                        </div>
